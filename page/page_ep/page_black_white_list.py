...
class PageBlackWhiteList(Page):
    # 前
    menu_message = Element(xpath='//div[@class="ant-menu-submenu-title"]/span[text()="用户管理"]', describe='消息平台')
    menu_bw_list = Element(xpath='//li//a[text()="黑白名单"]', describe='黑白名单菜单')
    menu_bw_lists = Elements(xpath='//li//a[text()="黑白名单"]', describe='判断黑白名单菜单是否存在')
    # 启动黑白名单
    ...
    ipt_black_list_name = Element(xpath='//input[@placeholder="请填写黑名单名称 ..."]', describe='黑名单名称输入框')
    ipt_white_list_name = Element(xpath='//input[@placeholder="请填写白名单名称 ..."]', describe='白名单名称输入框')
